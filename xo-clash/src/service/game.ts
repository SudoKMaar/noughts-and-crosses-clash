...
class GameService {
  ...
  public async onGameStarted(socket: Socket, callback: (data: any) => void) {
    socket.on("start_game", callback);
  }

  public async onGameWin(socket: Socket, callback: (data: any) => void) {
    socket.on("on_game_win", callback);
  }

  public async onMove(socket: Socket, callback: (data: any) => void) {
    socket.on("move_", callback);
  }

  public async onRematch(socket: Socket, callback: (data: any) => void) {
  ...
  public async leave(socket: Socket, room: string) {
    socket.emit("leave", { room });
  }

  public async onLeave(socket: Socket, callback: (data: any) => void) {
